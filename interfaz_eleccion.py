...
# Ventana principal
def crear_ventana(): #Crear la ventana principal
    ventana = tk.Tk()
...
def primer_titulo(ventana): #Crear el primer titulo (CARA BLANCA)
...
def finalizar(): #Finalizar el programna, se llama para el boton de finalizar.
    ventana.quit()  # Finaliza la aplicación
        
def main(): #Funcion main para importar:)
    global ventanas_abiertas,cara_actual,cubo,ventana,frame,botones,frame_botones,botones_nav
    ventanas_abiertas = []
    cara_actual = 0
    cubo = CuboColores()
    ventana = crear_ventana()        # Crea la ventana principal
    primer_titulo(ventana)           # Añadir el título
    frame = crear_frame(ventana)     # Crear el frame para los botones
    botones = botones_cubo(frame)    # Crear los botones del cubo
    poner_color(botones, cubo, cara_actual)  # Configurar los botones para que el usuario seleccione colores
    frame_botones = tk.Frame(ventana)  # Crear el frame para los botones de navegación
    frame_botones.pack(pady=20)         # Empacar el frame de los botones
    botones_nav = botones_siguiente(frame_botones, ventana, cubo)  # Crear los botones de navegación
    bloquear_boton(botones_nav, cara_actual)  # Bloquear/Desbloquear botones según la cara actual
...
def select_colors():
    caras = main() #Aqui solo lo estoy probando.
    i = 0
    for cara in caras:
        separadores = ["Blanca","Roja","Azul","Verde","Naranja","Amarilla"]
        print(separadores[i])
        for fila in cara:
            print(fila)
        i += 1
